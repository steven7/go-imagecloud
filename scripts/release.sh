...
cd $GOPATH/src/go-imagecloud
echo "=== Releasing imagecloud ==="
echo "  Deleting the local binary if it exists (so it isn't uploaded)..."
rm imagecloud
echo "  Done!"

echo "  Deleting existing code..."
ssh root@157.245.126.199 "rm -rf /root/go/src/go-imagecloud"
...
rsync -avr --exclude '.git/*' --exclude 'tmp/*' \
  --exclude 'image/*' ./ \
  root@157.245.126.199:/root/go/src/go-imagecloud/
...
ssh root@157.245.126.199 'export GOPATH=/root/go; \
  ...
  /usr/local/go/bin/go build -o ./server \
    $GOPATH/src/go-imagecloud/*.go'
echo "  Code built successfully!"

echo "  Moving assets..."
ssh root@157.245.126.199 "cd /root/app; \
  cp -R /root/go/src/go-imagecloud/assets ."
echo "  Assets moved successfully!"

echo "  Moving views..."
ssh root@157.245.126.199 "cd /root/app; \
  cp -R /root/go/src/go-imagecloud/views ."
echo "  Views moved successfully!"

echo "  Moving Caddyfile..."
ssh root@157.245.126.199 "cd /root/app; \
  cp /root/go/src/go-imagecloud/Caddyfile ."
echo "  Views moved successfully!"


echo "  Restarting the server..."
ssh root@157.245.126.199 "sudo service imagecloud restart"
...
echo "==== Done releasing imagecloud ===="